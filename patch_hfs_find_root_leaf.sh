...
echo "[patch_hfs_find_root_leaf] limpiando BOM/CRLF..."
sed -i $'1s/^\xEF\xBB\xBF//; s/\r$//' payload_hfsplus_boot.c 2>/dev/null || true

echo "[patch_hfs_find_root_leaf] reescribiendo hfsplus_find_root_file() con fallback de nodos..."
...
awk '
...

  {
    ...
    if (!in_find && $0 ~ /static int hfsplus_find_root_file\(const char\* name, ForkMeta\* out_fork\)\{/){
      ...
      print "";
      print "            /* Debug opcional: listar entradas de la raíz */";
      print "            /*";
      print "            puts(\"[hfs+] cat entry: parent=\");";
      print "            putu32(p_id);";
      print "            puts(\" name='\");";
      print "            puts(tmp);";
      print "            puts(\"\\' type=\");";
      print "            putu32((uint32_t)rtype);";
      print "            puts(\"\\r\\n\");";
      print "            */";
      print "";
...
echo "[patch_hfs_find_root_leaf] listo. Ahora ejecuta: ./build_run.sh"
